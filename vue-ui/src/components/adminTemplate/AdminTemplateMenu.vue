                ...
                <div class="ques-list">
                  <div class="ques-serial">
                    ...
                    <div class="ques-img" v-show="type != 'SYSTEM'">
                      ...
                      <img src="../../assets/adminPic/deleteProblem.png" @click="deleteMainQues(item2.id)" alt="" v-show="item2.editable">
                    </div>
...
export default {
  ...
  methods: {
    handleSelect(questionId) {
      if (!this.deleteFlag) {
        this.$emit("getId",questionId.toString())
      }
      this.deleteFlag = false
    },
    addMainQuestion(stepNo) {
      let newMainQues = {}
      newMainQues.templateId = this.templateId
      ...
      this.$http.post(`/api/admin/questionnaire/${this.templateId}/question/new`,newMainQues).then(res => {
        if (res.status == 200) {
    ...
    editMainQuestion(id) {
      let editQues = {}
      editQues.id = id
      editQues.mainQuestionId = id
      editQues.content = this.editMainQues[id]
      this.$http.post(`/api/admin/questionnaire/${this.templateId}/question/${id}`,editQues).then(res => {
        if (res.status == 200) {
          this.$emit("getId",res.data.question.id.toString())
    ...
    upRecordMainQues(index,step) {
      ...
      mainQuesRecorder.templateId = this.templateId
      ...
      this.$http.post(`/api/admin/questionnaire/${this.templateId}/question/reorder`,mainQuesRecorder).then(res => {
        if (res.status == 200) {
          if(index > 0) {
            this.$emit("getId",mainQuesList[index-1].toString())
            this.$emit("getEditFlag",true)
          } else {
            this.$emit("getId",mainQuesList[mainQuesList.length-1].toString())
            this.$emit("getEditFlag",true)
          }
          this.menuList = res.data.principleAssessments[this.principleMap[this.principle]].stepList
      ...
      this.$http.post(`/api/admin/questionnaire/${this.templateId}/question/reorder`,mainQuesRecorder).then(res => {
        if (res.status == 200) {
          if(index < mainQuesList.length-1) {
            ...
            this.$emit("getId",mainQuesList[0].toString())
            this.$emit("getEditFlag",true)
          }
